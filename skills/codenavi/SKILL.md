...
name: codenavi
description: >
  Your pathfinder for navigating unknown codebases. Investigates with precision,
  implements surgically, and never assumes — if it doesn't know, it says so.
  Maintains a .notebook/ knowledge base that grows across sessions, turning
  every discovery into lasting intelligence. Summons available skills, MCPs,
  and docs when the mission demands.
  Trigger: Use when fixing bugs, implementing features, refactoring,
  investigating flows, or any development task in unfamiliar territory, such
  as "fix this", "implement this", "how does this work", "investigate this
  flow", or "help me with this code". Do NOT use for greenfield scaffolding,
