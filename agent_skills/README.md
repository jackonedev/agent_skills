...
From the **root of your target repository** (the repo you want to add Agent Skills to):

```bash
# Install latest GitHub release (fallback: main)
...
- `agent_skills/README_TEMPLATE.md` (copy/adapt into your repo root `README.md`)

By default, existing `skills/` or `agent_skills/` directories are backed up with a timestamp suffix.

## Update

Re-run the installer with the desired `--ref` (or without it for latest release), then refresh Auto-invoke tables:
...
- docs-writter: (Development) Write, review, and edit documentation files with consistent structure, tone, and technical accuracy. Use when creating docs, reviewing markdown files.
